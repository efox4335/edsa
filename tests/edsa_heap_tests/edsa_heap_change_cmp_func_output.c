...
int main(void)
{
	...
	if(ret_val != DATA_AMOUNT){
		printf("edsa_heap_change_cmp_func_output.c: sorted data returned at index %ld not expected expected %ld instead got %ld\n",
		ret_val,
		ret_val,
		data[ret_val]);
		edsa_heap_free(heap);
	...
	if(ret_val != DATA_AMOUNT){
		printf("edsa_heap_change_cmp_func_output.c: scrambled data returned at index %ld not expected expected %ld instead got %ld\n",
		ret_val,
		ret_val,
		data[ret_val]);
		edsa_heap_free(heap);
	...
	if(ret_val != DATA_AMOUNT){
		printf("edsa_heap_change_cmp_func_output.c: randomized data returned at index %ld not sorted got values %ld and %ld\n",
		ret_val,
		data[ret_val - 1],
		data[ret_val]);
		edsa_heap_free(heap);
