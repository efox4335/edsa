...
int main(void)
{
	...
	edsa_heap_build(heap, data, DATA_AMOUNT);
	...
	if(ret_val != DATA_AMOUNT){
		printf("edsa_heap_build_output.c: sorted data returned at index %ld not expected expected %ld instead got %ld\n",
		ret_val,
		ret_val,
		data[ret_val]);
		edsa_heap_free(heap);
	...
	if(ret_val != DATA_AMOUNT){
		printf("edsa_heap_build_output.c: scrambled data returned at index %ld not expected expected %ld instead got %ld\n",
		ret_val,
		ret_val,
		data[ret_val]);
		edsa_heap_free(heap);
	...
	if(ret_val != DATA_AMOUNT){
		printf("edsa_heap_build_output.c: randomized data returned at index %ld not sorted got values %ld and %ld\n",
		ret_val,
		data[ret_val - 1],
		data[ret_val]);
		edsa_heap_free(heap);
	...
	//checks weather the heap can be used as normal
	set_arr(data, DATA_AMOUNT);

	edsa_heap_build(heap, data, DATA_AMOUNT / 2);

	for(int i = DATA_AMOUNT / 2; i < DATA_AMOUNT; ++i){
