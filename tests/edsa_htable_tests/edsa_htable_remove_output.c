//calls edsa_htable_remove() for all data with key values under REMOVE_INDEX
#include "edsa.h"
#include <stdio.h>
#include <stdlib.h>

enum{KEY_ARR_ELE = 4, DATA_ARR_ELE = 100, REMOVE_INDEX = 500};
...
//checks data from REMOVE_INDEX to size
//returns index of failed data
//returns size on pass
size_t check_removed_data_arr(data_type *arr, size_t size)
{
...
int main(void)
{
	...
	for(int i = 0; i < REMOVE_INDEX; ++i){
		edsa_htable_remove(htable, &(key[i]));
	}

	reset_data_arr(data, DATA_AMOUNT);//set to make sure edsa_htable_read() outputs data

	for(int i = 0; i < DATA_AMOUNT; ++i){
		ret_val = edsa_htable_read(htable, &(key[i]), &(data[i]));

		if(i < REMOVE_INDEX && ret_val != EDSA_HTABLE_READ_NO_ENTRY){
			printf("edsa_htable_remove_output.c: failed to remove data with key %ld\n",
			key[i].key_arr[0]);
	...
	if(ret_val != DATA_AMOUNT){
		printf("edsa_htable_remove_output.c: key at index %ld not expected expected %ld instead got %ld\n",
		ret_val,
		ret_val,
	...
	ret_val = check_removed_data_arr(data, DATA_AMOUNT);

	if(ret_val != DATA_AMOUNT){
		printf("edsa_htable_remove_output.c: data returned at index %ld not expected expected %ld instead got %ld\n",
		ret_val,
		ret_val,
	...
	//to check if the htable can be used normaly after entrys are removed
	set_key_arr(key, DATA_AMOUNT);
	set_data_arr(data, DATA_AMOUNT);
	...
	if(ret_val != DATA_AMOUNT){
		printf("edsa_htable_remove_output.c: key inputed after removal at index %ld not expected expected %ld instead got %ld\n",
		ret_val,
		ret_val,
		key[ret_val].key_arr[0]);
		edsa_htable_free(htable);
	...
	if(ret_val != DATA_AMOUNT){
		printf("edsa_htable_remove_output.c: data inputed after removal returned at index %ld not expected expected %ld instead got %ld\n",
		ret_val,
		ret_val,
		data[ret_val].data_arr[0]);
		edsa_htable_free(htable);
